...
select
  ...
  coalesce(live_counts.live_listing_count, 0) as landlord_live_listing_count,
  case
    when coalesce(live_counts.live_listing_count, 0) >= 3 then 'Professional Landlord'
    else 'Private Landlord'
  end as landlord_type
from public.properties p
join public.profiles pr on pr.id = p.landlord_id
left join lateral (
  select count(*)::int as live_listing_count
  from public.properties live_property
  where live_property.landlord_id = p.landlord_id
    and live_property.status = 'live'::public.property_status
) live_counts on true
